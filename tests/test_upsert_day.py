...
class TestUpsertDayInsert:
    ...
    def test_creates_table_and_inserts_rows(self, snowflake_con) -> None:
    ...
    def test_return_value_matches_row_count(self, snowflake_con) -> None:
        rows = [
    ...
    def test_raw_data_is_valid_json(self, snowflake_con) -> None:
...
class TestUpsertDayEmpty:
    ...
    def test_empty_list_returns_zero(self, snowflake_con) -> None:
        count = _upsert_day(snowflake_con, "sleep", [], date(2024, 6, 1))
        assert count == 0

    def test_empty_creates_table(self, snowflake_con) -> None:
    ...
    def test_none_rows_returns_zero(self, snowflake_con) -> None:
...
class TestUpsertDayIdempotent:
    ...
    def test_same_day_replaces_rows(self, snowflake_con) -> None:
        day = date(2024, 6, 1)
    ...
    def test_different_days_coexist(self, snowflake_con) -> None:
    ...
    def test_rerun_does_not_duplicate(self, snowflake_con) -> None:
        day = date(2024, 6, 1)
        row = [{"id": "same"}]
...
class TestUpsertDayPartitionDate:
    ...
    def test_adds_partition_date(self, snowflake_con) -> None:
        day = date(2024, 6, 15)
...
class TestUpsertDayInvalidTable:
    ...
    def test_invalid_table_raises(self, snowflake_con) -> None:
        with pytest.raises(ValueError, match="Invalid table name"):
            _upsert_day(snowflake_con, "not_a_table", [{"x": 1}], date(2024, 6, 1))

    def test_sql_injection_attempt(self, snowflake_con) -> None:
        with pytest.raises(ValueError, match="Invalid table name"):
            _upsert_day(
                snowflake_con,
