...
def sample_report_data() -> ReportData:
    ...
    return ReportData(
        ...
        areas_to_improve=[
            AreaToImprove(
                "steps",
                "Declined 8.3% over period",
                8200.0,
                "Try a daily walk after lunch",
            ),
        ],
        sleep_summary=SleepSummary(
            avg_duration_hrs=7.5,
            avg_efficiency=88.0,
            avg_hrv=42.0,
            avg_hr=56.0,
            deep_pct=20.0,
...
class TestRenderReport:
    ...
    def test_returns_html_string(self, sample_report_data, sample_charts) -> None:
        """Output should be an HTML document."""
        html = render_report(sample_report_data, sample_charts)
        assert "<!DOCTYPE html>" in html
        assert "</html>" in html

    def test_contains_period_header(self, sample_report_data, sample_charts) -> None:
        """Should display 'Weekly Health Report' for weekly period."""
        html = render_report(sample_report_data, sample_charts)
        assert "Weekly Health Report" in html

    def test_contains_date_range(self, sample_report_data, sample_charts) -> None:
        """Should display the date range in the header."""
        html = render_report(sample_report_data, sample_charts)
        assert "January 06" in html
        assert "January 12, 2025" in html

    def test_contains_metric_table(self, sample_report_data, sample_charts) -> None:
    ...
    def test_monthly_period_type(self, sample_charts) -> None:
    ...
    def test_handles_no_workout_data(self, sample_charts) -> None:
    ...
    def test_contains_generated_timestamp(
        self, sample_report_data, sample_charts
    ) -> None:
