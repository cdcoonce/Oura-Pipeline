...
Build the data access layer that queries DuckDB mart and staging tables for a given date range and returns typed Polars DataFrames. These functions bridge the database and the pure analysis layer from Phase 1.

## Dependencies

- Phase 1 complete (analysis dataclasses exist, used for type context)

## Files to Create

| File                                         | Purpose                                              |
| -------------------------------------------- | ---------------------------------------------------- |
| `tests/test_report_data.py`                  | Tests for SQL query functions using in-memory DuckDB |
| `src/dagster_project/reports/report_data.py` | SQL queries returning Polars DataFrames              |
...
All queries must use **parameterized queries** — not f-strings:
...
Use `pl.from_arrow(result.fetch_arrow_table())` for zero-copy conversion from DuckDB's Arrow output to Polars DataFrame. This is the most efficient path and avoids going through pandas.

### Empty Result Handling

When DuckDB returns 0 rows, `fetch_arrow_table()` still returns a typed Arrow table with correct schema. The Polars conversion will produce an empty DataFrame with correct column types. No special handling needed — but the tests should verify this behavior.
...
def con():
    ...
    con.execute("""
        ...
        SELECT
            '2025-01-01'::DATE + INTERVAL (i) DAY AS day,
    ...
    con.execute("""
        ...
        SELECT
            'workout_' || i AS id,
            '2025-01-01'::DATE + INTERVAL (i * 2) DAY AS day,
            ...
            '2025-01-01'::DATE + INTERVAL (i * 2) DAY AS partition_date
...
1. All tests in `test_report_data.py` pass
2. All SQL queries use parameterized `?` placeholders — no f-string interpolation for values
3. All functions return `pl.DataFrame` (not pandas)
4. Empty date ranges return empty DataFrames with correct column schemas
5. Results are ordered by `day` ascending
6. Type hints on all function signatures, numpy-style docstrings
7. `uv run pytest tests/test_report_data.py -v` passes with 0 failures
